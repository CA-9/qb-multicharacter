local QBCore = exports['qb-core']:GetCoreObject({ 'Shared', 'Functions', 'Commands', 'Player' })
local sharedStarterItems = QBCore.Shared.StarterItems
...
local function IsCharacterLocked(citizenid)
    return Config.LockedCharacters[citizenid] == true
end
...
RegisterNetEvent('qb-multicharacter:server:disconnect', function()
    local src = source
    DropPlayer(src, Lang:t('misc.droppedplayer'))
end)
...
RegisterNetEvent('qb-multicharacter:server:deleteCharacter', function(citizenid)
    ...
    if IsCharacterLocked(citizenid) then
        TriggerClientEvent('QBCore:Notify', src, 'This character is locked and cannot be deleted', 'error')
        return
    end
...
QBCore.Commands.Add('deletechar', Lang:t('commands.deletechar_description'), { { name = Lang:t('commands.citizenid'), help = Lang:t('commands.citizenid_help') } }, false, function(source, args)
    if args and args[1] then
        if IsCharacterLocked(tostring(args[1])) then
            TriggerClientEvent('QBCore:Notify', source, 'This character is locked and cannot be deleted', 'error')
            return
        end
